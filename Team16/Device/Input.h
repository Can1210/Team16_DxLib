...
protected:
	Input(void);  //コンストラクタ
	virtual ~Input(void); //デスストラクタ
public:
	friend class Singleton<Input>;     //Singletonでのインスタンス作成は許可


#pragma region キーボード関連
	// キーが押された瞬間
	bool isKeyDown(KEYCORD key);
	//キーが押されている間
	bool isKeyState(KEYCORD key);
	//キーが離された瞬間
	bool isKeyUp(KEYCORD key);
#pragma endregion
...
#pragma region ゲームパッド関係
	...
	//ボタンが押された瞬間
	bool isGamePadBottonDown(GAMEPAD_KEYCORD button, int padNum);
	//ボタンが押されている間
	bool isGamePadBottonState(GAMEPAD_KEYCORD button, int padNum);
	//ボタンが離されて瞬間
	bool isGamePadBottonUp(GAMEPAD_KEYCORD button, int padNum);

	//ボタンが押された瞬間
	bool isGamePadCrossPadDown(GAMEPAD_KEYCORD button, int padNum);
	//ボタンが押されている間
	bool isGamePadCrossPadState(GAMEPAD_KEYCORD button, int padNum);
	//ボタンが離されて瞬間
	bool isGamePadCrossPadUp(GAMEPAD_KEYCORD button, int padNum);
