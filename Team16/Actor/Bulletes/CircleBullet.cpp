...
void CircleBullet::update(float deltaTime)
{
	...
	{
		//moveTime += deltaTime * 4.0f;//‰ñ“]—p
		//x = 2.0f * radius* cos(moveTime* rotateSpeed);
		//y = 3.0f * radius * sin(moveTime* rotateSpeed);
		// 	b_mVelocity = Vector2(x, y);
		//b_mVelocity.y += 4.0f;
		//
		//b_mPosittion += b_mVelocity;//‰ñ“]—p

		b_mVelocity = Vector2(0.0f, 1.0f);//^‚Á’¼‚®—p
		b_mPosittion += b_mVelocity * deltaTime * b_mSpeed;//^‚Á’¼‚®—p
	}
